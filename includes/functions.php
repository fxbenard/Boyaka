 ...
 * Helper Functions
 *
 * @package     FxB\Bloom French\Functions
 * @since       1.0.0
 */
...
if(!function_exists('fxb_license_menu')) {
	function fxb_license_menu() {
		add_plugins_page( __( 'FxB trads', 'bloom-french' ), __( 'FxB trads', 'bloom-french' ), 'manage_options', 'fbx-trads-license', 'fxb_license_page' );
	}
...
function fbx_bloom_key($license, $status, $content_key) {
	$plugin = 'Bloom';

	$content_key .= '<tr valign="top">
		<th scope="row" valign="top">'
			. __( 'License Key', 'bloom-french' ) . ' ' . $plugin .'
		</th>
		<td>
			<input id="fxb_bloom_license_key" name="fxb_bloom_license_key" type="text" class="regular-text" value="' . esc_attr__( $license ) . '" />
			<label class="description" for="fxb_bloom_license_key">' . __( 'Enter your license key', 'bloom-french' ) . '</label>
		</td>
	</tr>';
	if ( false !== $license ) { 
		$content_key .= '<tr valign="top">
		<th scope="row" valign="top">
			' . __( 'Activate License', 'bloom-french' ) . '
		</th>
		<td>';
		if ( false !== $status && "valid" == $status ) {
			$content_key .= '<span style="color:green;">' . __( 'active', 'bloom-french' ) . '</span>
				' . wp_nonce_field( "fxb_bloom_nonce", "fxb_bloom_nonce" ) . '
				<input type="submit" class="button-secondary" name="edd_license_deactivate" value="' . __( 'Deactivate License', 'bloom-french' ) .'"/>';
		} else {
			$content_key .= wp_nonce_field( "fxb_bloom_nonce", "fxb_bloom_nonce" ) .'
				<input type="submit" class="button-secondary" name="edd_license_activate" value="' . __( 'Activate License', 'bloom-french' ) .'"/>';
		}
		$content_key .= '</td>
		</tr>';
	}
...
add_action('fbx_key_setting', 'fbx_bloom_key', 10, 3);

if(!function_exists('fxb_bloom_register_option')) {
...
if(!function_exists('fxb_bloom_activate_license')) {
...
if(!function_exists('fxb_bloom_deactivate_license')) {
...
if(!function_exists('fxb_bloom_init')) {
	/* Hook to init
	------------------------------------------*/
